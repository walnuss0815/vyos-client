...
| `CONTAINER_UPDATE_BACKGROUND_CHECK_ENABLED` | `false` | Whether the same check runs automatically, on a schedule, for every configured container - raising an in-app Notification for anything it finds. Disabled by default, and requires `CONTAINER_UPDATE_CHECKS_ENABLED=true` too (startup error otherwise) - this automates that same registry-contacting check, so it shouldn't run unattended unless the manual version has also been explicitly opted into. See [architecture.md](architecture.md#background-checks-container_update_background_check_enabled). |
| `CONTAINER_UPDATE_CHECK_CRON` | `0 3 * * *` (daily at 03:00) | Standard 5-field cron schedule for the background check above. Only meaningful when `CONTAINER_UPDATE_BACKGROUND_CHECK_ENABLED=true`; ignored (with a startup warning) otherwise. Validated at startup - an invalid expression fails fast rather than silently never registering the job. |
