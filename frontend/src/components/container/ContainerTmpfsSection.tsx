...
import type { ContainerTmpfs } from '../../lib/containerTypes'
 ...
 * .tsx in draft mode (via onAdd/onRemove) - see this component's own
 * prop doc comments. */
export default function ContainerTmpfsSection({
  containerName,
  tmpfs,
  onAdd,
  onRemove,
  ...
  /** Overrides what "Add tmpfs mount" does, in place of the default
   * "immediately queue the real set ops" - see ChipList.tsx's onAdd
   * doc comment for the general rationale. `tmpfs` must then be
   * whatever local state onAdd/onRemove write to. */
  onAdd?: (id: string, destination: string, size: string) => void
  ...
  const taken = tmpfs.some((t) => t.id === trimmedId)
  const valid = trimmedId !== '' && !taken

  function submit() {
    if (!valid) return
    if (onAdd) {
      onAdd(trimmedId, destination, size)
    } else {
      const ops = addTmpfsOps(containerName, trimmedId, destination, size)
      for (const op of ops) {
  ...
  function remove(tmpfsId: string) {
    if (onRemove) {
      onRemove(tmpfsId)
      return
    }
    const op = removeTmpfsOp(containerName, tmpfsId)
    ...
    <div>
      {tmpfs.map((t) => (
        <div key={t.id} className="mb-1 flex items-center justify-between rounded border border-surface-border p-2">
          ...
          <button onClick={() => remove(t.id)} className="text-xs text-slate-500 hover:text-danger-500">
            Remove
          </button>
        </div>
      ))}
      {tmpfs.length === 0 && <p className="text-xs text-slate-500">No tmpfs mounts.</p>}
