...
| `INGRESS_ENABLED` | `false` | Whether the Ingress nav group/page is shown, letting a logged-in user reach a web UI elsewhere on the router's own network through this app (`/ingress/<name>/...`), without opening a separate port for it. Disabled by default — unlike everything else in this app, this lets the backend reach hosts beyond VyOS's own API. See [architecture.md](architecture.md#ingress). |
| `INGRESS_DATA_DIR` | `/data` | Directory ingress entries (name, target URL, request headers) are persisted to, as a single `ingress.json` file. Only meaningful when `INGRESS_ENABLED=true`, but the default is applied unconditionally. **Must be a persistent mounted volume** — entries are lost on every container restart otherwise, since the directory is created automatically if missing but its contents are not VyOS configuration and have no other backup. |
